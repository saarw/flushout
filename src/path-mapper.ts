/**
 * Records and performs path mappings when different proxies create the
 * same IDs so one's commands need to get remapped.
 */
  ...
  public get(path: string[]): string[] | undefined {
    ...
    while (p.length > 0) {
      ...
      if (mappedSection != undefined) {
        const mappedPath = mappedSection.concat(
          path.slice(p.length, path.length)
        );
